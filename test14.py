        ...
        try:
            for ticker in ori_tickers:
                ...
                if target_p < current_p and man5 < ma5 and ma10 < ma5:
                    if krw > 980000 and bct_balances == 0:
                        upbit.buy_market_order(ticker, 20000)                       
    ...
    while True:
        try:
            for ticker in ori_tickers:
                ...
                bct_balances = upbit.get_balance(ticker)
                if 0 < bct_balances:
                    if bct_balances < (20000/current_p):
                        if current_p < buy_p * 0.97:
                            upbit.buy_market_order(ticker, 20000)
                        if current_p > buy_p * 1.02:
                            upbit.sell_market_order(ticker, bct_balances*0.999)
                    elif (20000/current_p) < bct_balances < (40000/current_p):
                        if current_p < buy_p * 0.97:
                            upbit.buy_market_order(ticker, 40000)
                        if current_p > buy_p * 1.02:
                            upbit.sell_market_order(ticker, bct_balances*0.999)
                    elif (40000/current_p) < bct_balances < (80000/current_p):
                        if current_p < buy_p * 0.97:
                            upbit.buy_market_order(ticker, 80000)
                        if current_p > buy_p * 1.02:
                            upbit.sell_market_order(ticker, bct_balances*0.999)
                    elif (80000/current_p) < bct_balances < (160000/current_p):
                        if current_p < buy_p * 0.97:
...
while True:
    now = datetime.datetime.now()
    if now.hour == 22 and now.minute == 10 and 1 <=now.second <= 10:
        op_mode = True
        print("시작")
